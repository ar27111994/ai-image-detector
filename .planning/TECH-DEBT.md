...
- Five UI/content modules had zero unit tests → now covered; the suite grew to 431 tests with
  coverage spanning all of src/ at 98.6% lines / 91.4% branches / 98.0% functions (90% gate).
...
- Concurrent model downloads raced (a timed-out onboarding retry started a second ~311MB
  ...
  `ensureModel` promise, raced against a deadline so a stalled download abandons and a retry
  starts fresh instead of blocking until the SW restarts (mirrors the session/offscreen guards).
