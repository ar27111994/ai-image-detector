...
- [x] Post-release audit + hardening (2026-08-16): 32 findings fixed — accuracy docs corrected to
      ...
      crash recovery, and unit tests for all previously-untested modules (430 tests / 34 files).
- [x] PR #1 review fixes (2026-08-17): concurrent-download dedup guard + WebGPU session-leak
      release (both with regression tests); CodeQL hygiene (PNG iTXt dead store, pack.mjs TOCTOU);
      conversion-toolchain pins bumped (onnx 1.21.0, pillow 12.3.0, transformers 5.5.0) to clear
      the dependency-review gate.
