...
| Claimed balanced accuracy | 92.5% (author-reported, model's own 15K test) / 83.3% on 31 imgs / **76.7% on 43 imgs** | **84.5%** (calibrated raw) / **80.6%** (augmented) @ 0.65                                                          |
| Calibration               | Monotonic score normalization (p=0.5 → 65%); not an empirical probability               | Platt logistic fit on a held-out train split (measured lift 80.7%→83.8%)                                           |
